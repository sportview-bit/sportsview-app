...
const Shell: React.FC = () => {
  ...
  const [user, setUser] = useState<User | null>(() => {
    const raw = localStorage.getItem('svtz_staff');
    return raw ? JSON.parse(raw) : null;
  });

  useEffect(() => {
    if (user) localStorage.setItem('svtz_staff', JSON.stringify(user));
    else localStorage.removeItem('svtz_staff');
  }, [user]);
  ...
  // Landing — Prioritize Wafanyakazi (Staff) door as shown in screenshots.
  return (
