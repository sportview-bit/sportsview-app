...
export const AdminDashboard: React.FC<AdminProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#0B0F14] text-[#F3F6F9]">
      <header className="border-b border-[#232D3A] px-6 py-4 flex items-center justify-between sticky top-0 bg-[#0B0F14]/90 backdrop-blur z-10">
        <div>
          ...
          <h1 className="text-xl font-bold" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Admin Dashboard</h1>
        </div>
        <button onClick={onExit} className="text-sm text-[#8B98A8] hover:text-[#FF5468] transition">Sign out</button>
      </header>

      <div className="px-6 py-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
          <div className="bg-[#121821] border border-[#232D3A] rounded-xl px-5 py-4">
            <p className="text-xs text-[#8B98A8] uppercase tracking-wider">Rooms</p>
            <p className="text-2xl font-bold font-mono mt-1">{rooms.length}</p>
          </div>
          <div className="bg-[#121821] border border-[#232D3A] rounded-xl px-5 py-4">
            <p className="text-xs text-[#8B98A8] uppercase tracking-wider">Managers</p>
            <p className="text-2xl font-bold font-mono mt-1 text-[#34D399]">{managers.length}</p>
          </div>
          <div className="bg-[#121821] border border-[#232D3A] rounded-xl px-5 py-4">
            <p className="text-xs text-[#8B98A8] uppercase tracking-wider">Sponsors</p>
            <p className="text-2xl font-bold font-mono mt-1 text-[#A78BFA]">{sponsors.length}</p>
          </div>
          <div className="bg-[#121821] border border-[#232D3A] rounded-xl px-5 py-4">
            <p className="text-xs text-[#8B98A8] uppercase tracking-wider">Entries Today</p>
            <p className="text-2xl font-bold font-mono mt-1">{totalEntries}</p>
          </div>
          <div className="bg-[#121821] border border-[#232D3A] rounded-xl px-5 py-4">
            <p className="text-xs text-[#8B98A8] uppercase tracking-wider">Revenue Today</p>
            <p className="text-2xl font-bold font-mono mt-1 text-[#34D399]">{totalMoney.toLocaleString()} TZS</p>
          </div>
        </div>

        <div className="flex gap-2 mb-6 border-b border-[#232D3A] overflow-x-auto">
          {(['overview', 'matches', 'rooms', 'sponsors'] as const).map(t => (
            <button
              ...
              className={`px-4 py-2 text-sm font-semibold capitalize border-b-2 -mb-px transition whitespace-nowrap ${
                tab === t ? 'border-[#F2B705] text-[#F2B705]' : 'border-transparent text-[#8B98A8] hover:text-white'
              }`}
            >
        ...
        {tab === 'overview' && (
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-[#121821] border border-[#232D3A] rounded-xl p-6">
                ...
                {[...rooms].sort((a, b) => b.todayRevenue - a.todayRevenue).slice(0, 8).map(r => (
                  <div key={r.id} className="flex justify-between items-center bg-[#0B0F14] border border-[#232D3A] rounded-lg px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-[#8B98A8]">{r.location} • {managerName2(r.managerId)}</p>
                    </div>
                    <p className="font-mono text-[#34D399] font-bold">{r.todayRevenue.toLocaleString()} TZS</p>
                  </div>
                ))}
                {rooms.length === 0 && <p className="text-sm text-[#8B98A8]">No rooms yet — add one under Rooms & Managers.</p>}
              </div>
            </div>
            <div className="bg-[#121821] border border-[#232D3A] rounded-xl p-6">
              ...
              <div className="space-y-2">
                {matches.slice(0, 8).map(m => (
                  <div key={m.id} className="flex justify-between items-center bg-[#0B0F14] border border-[#232D3A] rounded-lg px-4 py-3">
                    <p className="font-semibold text-sm">{m.homeTeam} vs {m.awayTeam}</p>
                    <p className="text-xs text-[#8B98A8]">{m.matchTime}</p>
                  </div>
                ))}
                {matches.length === 0 && <p className="text-sm text-[#8B98A8]">No matches posted yet.</p>}
              </div>
            </div>
          </div>
        )}
              ...
              <form onSubmit={handleCreateRoom} className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input type="text" placeholder="Manager Name" required value={managerName} onChange={e => setManagerName(e.target.value)}
                    className="bg-[#0B0F14] border border-[#232D3A] rounded-lg px-4 py-2 text-white outline-none focus:border-[#34D399] transition" />
                  <input type="text" placeholder="Manager Phone" required value={managerPhone} onChange={e => setManagerPhone(e.target.value)}
                    className="bg-[#0B0F14] border border-[#232D3A] rounded-lg px-4 py-2 text-white outline-none focus:border-[#34D399] transition" />
                </div>
                <p className="text-xs text-[#8B98A8] pt-1">Login the manager will use to sign in:</p>
                <div className="grid grid-cols-2 gap-3">
                ...
                {rooms.map(r => (
                  <div key={r.id} className="flex justify-between items-center bg-[#0B0F14] p-3 rounded-lg border border-[#232D3A]">
                    <div>
                      ...
                      <div className="text-xs font-mono text-[#8B98A8] mt-1">
        ...
        {tab === 'sponsors' && (
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-[#121821] border border-[#232D3A] p-6 rounded-2xl">
              ...
              <form onSubmit={handleCreateSponsor} className="space-y-3">
                <input type="text" placeholder="Sponsor / company name" required value={sponsorName} onChange={e => setSponsorName(e.target.value)}
                  className="w-full bg-[#0B0F14] border border-[#232D3A] rounded-lg px-4 py-2 text-white outline-none focus:border-[#A78BFA] transition" />
                <div className="grid grid-cols-2 gap-3">
                  <input type="number" placeholder="Amount sponsored (TZS)" required value={sponsorAmount} onChange={e => setSponsorAmount(e.target.value)}
                    className="bg-[#0B0F14] border border-[#232D3A] rounded-lg px-4 py-2 text-white outline-none focus:border-[#A78BFA] transition" />
                  <input type="number" placeholder="Profit share %" required value={sponsorShare} onChange={e => setSponsorShare(e.target.value)}
                    className="bg-[#0B0F14] border border-[#232D3A] rounded-lg px-4 py-2 text-white outline-none focus:border-[#A78BFA] transition" />
                </div>
                <p className="text-xs text-[#8B98A8] pt-1">Login the sponsor will use to sign in:</p>
                <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-[#121821] border border-[#232D3A] p-6 rounded-2xl">
              ...
              <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
                {sponsors.length === 0 && <p className="text-[#8B98A8] text-sm">No sponsors registered yet.</p>}
                {sponsors.map(s => (
                  <div key={s.id} className="flex justify-between items-center bg-[#0B0F14] p-3 rounded-lg border border-[#232D3A]">
                    <div>
                      <span className="font-bold text-[#A78BFA]">{s.name}</span>
                      <div className="text-xs text-[#8B98A8]">@{s.username} • {s.profitSharePercent}% profit share</div>
                      <div className="text-xs font-mono text-[#8B98A8] mt-1">{s.amountSponsored.toLocaleString()} TZS sponsored</div>
                    </div>
